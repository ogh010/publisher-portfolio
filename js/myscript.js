$(function(){
    var _pos = [100, 90, 70, 85, 45,65, 80, 30,45]; //내 스킬값 _index값으로 찾는거임
    var setId; //밑에 체크박스
    $("#section2 .link-wrap .link-unit .btn").mouseover(function(){
        ...
        $("#section2 .wave-wrap .unit").css("transform","translateY("+_posReverse+"%)"); //파도의 높이값 제어
        ...
        $("#section2 .fish").css("transform","translateY("+_posReverse+"%)").addClass("on"); 
        $("#section2 .fish").addClass("on");
        $("#section2 .fish2").addClass("on2")
    });
    $("#section2 .link-wrap .link-unit .btn").mouseout(function(){ //마우스아웃하면 파도가 밑으로
    ...
    $("#section2 .wave-wrap .unit").on("transitionend",function(){//setInterval을 멈추기 위한 애니메이션 끝시점end 체크(transition 움직임이 끝날때)
    ...
    /*section5 마지막*/
    $('#section5 .myInfo').mouseover(function(){
    ...
    $('#section5 .contactSns , #section5 .contactSns2').mouseover(function(){
        $('#section5 .contactSns ul li , #section5 .contactSns2 ul li').css({'animation-play-state':'paused'})
    }).mouseout(function(){
        $('#section5 .contactSns ul li , #section5 .contactSns2 ul li').css({'animation-play-state':'running'})
    })
    
    $('#section5 .ContactInner .ContactRight .form .inputBox input[type=text]').focus(function(){
        $(this).addClass('focus');
    }).blur(function(){
    ...
    function fadeInOutBG(n){
        ...
        $('#section3 .pBg .slideViewBg div').eq(n%4).stop().fadeIn(500);
        
        /*반응형 부분*/
        $('#section3 .pResponsive .slideViewBg2 .res').stop().fadeOut(100);
        $('#section3 .pResponsive .slideViewBg2 .res, #section3 .pResponsive .slideViewBg2 .res > *').eq(n%4).stop().fadeIn(100);
        
        
        /*포트폴리오 설명부분*/
        $('#section3 .pContainer .pText .slideView .con').stop().animate({'left':'-120%'},400);
        $('#section3 .pContainer .pText .slideView .con').eq(n%4).css('left','120%').stop().animate({'left':'0%'},500);
        /*포트폴리오 설명tab부분*/
        $('#section3 .pContainer .pText2 .slideView2 .con2').stop().animate({'left':'-120%'},400);
        $('#section3 .pContainer .pText2 .slideView2 .con2 ').eq(n%4).css('left','120%').stop().animate({'left':'0%'},500);
        
        /*포트폴리오 모니터부분*/
        $('#section3 .pContainer .pCont .pComputer .slideView2 div').stop().animate({'left':'-100%'},400);
        $('#section3 .pContainer .pCont .pComputer .slideView2 div').eq(n%4).css('left','100%').stop().animate({'left':'0%'},300);
    ...
    $('#section3 .indicator .prev').click(function(){
        ...
        $('.line .pLine').stop().animate({'height':posY+'px'});
        $('#section3 .pContainer .pText p').removeClass("on");
        $('#section3 .pContainer .pText p').eq(n%4).addClass("on");
        $('#section3 .pContainer .pText h1').removeClass("on");
        $('#section3 .pContainer .pText h1').eq(n%4).addClass("on");
    })
